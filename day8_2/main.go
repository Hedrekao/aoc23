package main

import (
	"aoc2023/utils"
	"fmt"
	"strings"
)

type Direction struct {
	left  string
	right string
}

func main() {
	content, err := utils.LoadTextFile("data/input8.txt")

	if err != nil {
		panic(err)
	}
	lines := strings.Split(content, "\r\n")
	instructions := make([]string, 0)
	directions := make(map[string]Direction)

	steps := 0

	for idx, line := range lines {
		if idx == 0 {
			instructions = parseInstruction(line)
			continue
		}
		if line == "" {
			continue
		}
		key, direction := parseDirection(line)
		directions[key] = direction

	}

	currentPlaces := make([]string, 0)

	for key := range directions {
		if key[len(key)-1:] == "A" {
			currentPlaces = append(currentPlaces, key)
		}
	}

	instructionsIdx := 0
	fmt.Println(currentPlaces)
	cycles := make([]int, 0)
	for _, place := range currentPlaces {
		instructionsIdx = 0
		currentPlace := place
		for currentPlace[2:] != "Z" {
			direction := directions[currentPlace]
			steps++
			if instructionsIdx >= len(instructions) {
				instructionsIdx = 0
			}
			instruction := instructions[instructionsIdx]
			if instruction == "L" {
				currentPlace = direction.left
			} else {
				currentPlace = direction.right
			}
			instructionsIdx++
		}
		cycles = append(cycles, steps)
		steps = 0
	}

	fmt.Println(cycles)
	lcm := lcm(cycles...)
	fmt.Println(lcm)
}

func parseInstruction(instruction string) []string {
	commands := strings.Split(instruction, "")

	result := make([]string, 0)
	result = append(result, commands...)
	return result

}

func parseDirection(direction string) (string, Direction) {
	splitted := strings.Split(direction, " = ")
	key := splitted[0]
	value := splitted[1]
	directions := strings.Split(value, ",")
	left := directions[0][1:]
	right := directions[1][1 : len(directions[1])-1]
	directionObj := Direction{left, right}
	return key, directionObj
}

func lcm(numbers ...int) int {

	var result int = 1

	for _, number := range numbers {
		result = lcmOfTwo(result, number)
	}

	return result

}

func lcmOfTwo(x int, y int) int {

	return (x * y) / gcd(x, y)

}

func gcd(x int, y int) int {

	for y != 0 {
		x, y = y, x%y
	}
	return x

}
