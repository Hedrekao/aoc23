...
func main() {
	...
	for rowIdx, row := range maze {
		for colIdx := range row {
			cellId := calculateCellId(colIdx, rowIdx, mazeWidth)
			if visited.Contains(cellId) {
				continue
			}
			crosses := 0
			tmpX, tmpY := colIdx, rowIdx

			for tmpX < mazeWidth-1 && tmpY < len(maze)-1 {
				valueChar := maze[tmpY][tmpX]
				cellId := calculateCellId(tmpX, tmpY, mazeWidth)
				if visited.Contains(cellId) && valueChar != 'L' && valueChar != '7' {
					crosses++
				}
				tmpX++
				tmpY++
